...
mean_mat <- all_data %>%
  group_by(label) %>%
  summarize(across(c("FirstPerson", "InnerThinking", "ThinkPositive", "ThinkNegative", "ThinkAhead", "ThinkBack", "Reasoning", "Share_SocTies", "Direct_Activity",
                     "Interacting", "Notifying", "LinearGuidance", "WordPicture", "SpaceInterval",
                     "Motion", "PastEvents", "TimeInterval", "ShiftingEvents", "Text_Coverage"), mean)) %>%
...
df_heat_prop <- df_heat_prop %>% mutate(genre_names = case_when(
...
genre_names = factor(genre_names, levels = c(
  "Press: Reporting",
  "Press: Editorial",
  "Press: Reviews",
  "Religion",
...
# Heatmap by genre
p1 <- ggplot(df_heat_prop, aes(x = cluster, y = genre_names, fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = round(proportion, 2)),
            color = "black", size = 3.5) +     # larger numbers on tiles
...
# Heatmap by category
p2 <- ggplot(df_heat_prop_cat, aes(x = cluster, y = category, fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = round(proportion, 2)),
            color = "black", size = 3.5) +     # larger numbers on tiles
  scale_fill_gradient(low = "white", high = "red") +
...
ggsave("Figures/cluster_genre_heatmaps.png", cluster_heatmaps, width = 12, height = 5)




var_means <- all_data %>% select(label, FirstPerson, InnerThinking, ThinkPositive, ThinkNegative, ThinkAhead, ThinkBack, Reasoning, Share_SocTies, Direct_Activity,
                     Interacting, Notifying, LinearGuidance, WordPicture, SpaceInterval,
                     Motion, PastEvents, TimeInterval, ShiftingEvents, Text_Coverage) %>%
  group_by(label) %>%
  summarize(across(everything(), mean)) %>%
  pivot_longer(-label, names_to = "feature", values_to = "mean_value")

var_means_plot <- ggplot(var_means, aes(x = label, y = mean_value, fill = label)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ feature, scales = "free_y") +
  labs(title = "Mean Feature Values by Cluster", x = "Cluster", y = "Mean Value") +
  theme_minimal() +
